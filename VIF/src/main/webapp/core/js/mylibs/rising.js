...
window.Rising = new (function($) {
	...
	function defaultSlider(self) {
		...
		for (i=0; i < this.totalSlides; i++) {
			cache.push($(_.sprintf('<img width="1024" height="%s" src="%s" />', this.config.height, self.sliderData[i].img)));
		}
...
//				  .show()
			...
			blind.html(cache[next]).stop().animate({width: '1024px'}, self.config.transitionTime, self.config.easing, function() {
	...
	function nivoSlider(self) {
		...
		_.each(self.sliderData, function(ob) {
			if (i != 0) images += _.sprintf('<img width="1024" height="%d" src="%s" />\n', self.sliderConfig.sliderHeight, ob.img);
	...
	function accordionSlider(self) {
		...
		_.each(self.sliderData, function(ob) {
			images.push(_.sprintf('<li style="width: %dpx"><img width="1024" height="%d" src="%s" /><span class="bottom-bar"><a href="%s">%s</a></span></li>', w, h, ob.img, ob.url, ob.title));
		});
	...
	this.isDesktop = function() {
		return parseInt(wrap.width()) == 1024; // + 60 = 960px
	}
